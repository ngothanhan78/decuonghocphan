...
const TongHopPanel = {
  ...
  open() {
    // Reset trạng thái mỗi lần vào module — tránh trạng thái cũ từ lần trước
    this._resetSteps();
    App.show('tonghop');
  ...
  _resetSteps() {
    ...
    // Khóa lại 2 tab sau
    ['tonghop', 'trichxuat'].forEach(tabId => {
      const btn = document.getElementById('thTab-' + tabId);
      if (!btn) return;
      btn.classList.remove('th-tab-unlocked');
      btn.classList.add('th-tab-locked');
    });

    // Reset label về trạng thái có khóa
    const btnTH = document.getElementById('thTab-tonghop');
    const btnTX = document.getElementById('thTab-trichxuat');
    if (btnTH) btnTH.textContent = '🗂 Sắp xếp & Tổng hợp 🔒';
    if (btnTX) btnTX.textContent = '🔍 Trích xuất 🔒';

    // Ẩn kết quả cũ
    const resultBox = document.getElementById('thTongHopResult');
    if (resultBox) resultBox.style.display = 'none';
    const trichBox = document.getElementById('thTrichXuatResult');
    if (trichBox) trichBox.style.display = 'none';

    // Xóa bảng kiểm tra cũ
